...
//
//#define jack_up_signal 52
//#define jack_down_signal 53
//#define LA_up_signal 50
//#define LA_down_signal 51
...
int calFactor_compression = 53; // Cal factor of the 50kg loadcell
...
float One_bar_grams = 27260; // 27260g ( 27.26kg) Smaller values translate to higher pressure values for the same mass (Eg. 10kg)
...
//    mould1_pressure = ( (loadcell_Compress1_val*2) / One_bar_grams) + 1 - reset_loadcell_compress_val;
//    mould2_pressure = ( (loadcell_Compress3_val*2) / One_bar_grams) + 1 - reset_loadcell_compress_val;
//    mould3_pressure = ( (loadcell_Compress5_val*2) / One_bar_grams) + 1 - reset_loadcell_compress_val;
//    mould4_pressure = ( (loadcell_Compress7_val*2) / One_bar_grams) + 1 - reset_loadcell_compress_val;
...
void setup(void) { 
  ...
  LoadCell_Filling.start(1); // load cells gets 1ms of time to stabilize
  LoadCell_Filling.setCalFactor(calFactor_filling); //Set calibration factor specific to loadcell

  LoadCell_Compress_1.begin();LoadCell_Compress_1.start(1);LoadCell_Compress_1.setCalFactor(calFactor_compression);
  LoadCell_Compress_2.begin();LoadCell_Compress_2.start(1);LoadCell_Compress_2.setCalFactor(calFactor_compression);
  LoadCell_Compress_3.begin();LoadCell_Compress_3.start(1);LoadCell_Compress_3.setCalFactor(calFactor_compression);
  LoadCell_Compress_4.begin();LoadCell_Compress_4.start(1);LoadCell_Compress_4.setCalFactor(calFactor_compression);
  LoadCell_Compress_5.begin();LoadCell_Compress_5.start(1);LoadCell_Compress_5.setCalFactor(calFactor_compression);
  LoadCell_Compress_6.begin();LoadCell_Compress_6.start(1);LoadCell_Compress_6.setCalFactor(calFactor_compression);
  LoadCell_Compress_7.begin();LoadCell_Compress_7.start(1);LoadCell_Compress_7.setCalFactor(calFactor_compression);
  LoadCell_Compress_8.begin();LoadCell_Compress_8.start(1);LoadCell_Compress_8.setCalFactor(calFactor_compression);
...
//  if (loadcell_Filling_val<0){loadcell_Filling_val = loadcell_Filling_val * (-1);}
...
void loop(void) {
  ...
  if( (digitalRead(jack_up_btn) == HIGH) && enable_overrride==true){digitalWrite(jack_up_signal, HIGH);
  }else{digitalWrite(jack_up_signal, LOW);}

  if( (digitalRead(jack_down_btn) == HIGH) && enable_overrride==true){digitalWrite(jack_down_signal, HIGH);
  }else{digitalWrite(jack_down_signal, LOW);}
